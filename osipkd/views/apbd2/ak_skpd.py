...
class AKSKPDBaseViews(TuBaseViews):
    def __init__(self, context, request):
        self.app = 'c100'
        TuBaseViews.__init__(self, context, request)
        
    @view_config(route_name="c100", renderer="../../templates/apbd/akskpd/home.pt")
    def c100(self):
...
#Jurnal Penerimaan           
class C102001View(TuBaseViews):
    def __init__(self, context, request):
        TuBaseViews.__init__(self, context, request)
        self.session['mod'] = 'C102001'

    @view_config(route_name="c102_001", renderer="../../templates/apbd/akskpd/C102001.pt")
    def c102_001(self):
        params = self.request.params
        if self.logged:
            return dict(datas=self.datas)
    ...
    @view_config(route_name="c102_001_frm", renderer="../../templates/apbd/akskpd/C102001_frm.pt")
    def c102_001_frm(self):
        req      = self.request
        params   = req.params
    ...
    @view_config(route_name="c102_001_act", renderer="json")
    def c102_001_act(self):
        req      = self.request
        params   = req.params
        url_dict = req.matchdict
...
#Jurnal Pengeluaran           
class C102002View(TuBaseViews):
    def __init__(self, context, request):
        TuBaseViews.__init__(self, context, request)
        self.session['mod'] = 'C102002'

    @view_config(route_name="c102_002", renderer="../../templates/apbd/akskpd/C102002.pt")
    def c102_002(self):
        params = self.request.params
        if self.logged:
            return dict(datas=self.datas)
    ...
    @view_config(route_name="c102_002_frm", renderer="../../templates/apbd/akskpd/C102002_frm.pt")
    def c102_002_frm(self):
        req      = self.request
        params   = req.params
        url_dict = req.matchdict
        self.datas['id'] = 'id' in url_dict and int(url_dict['id']) or 0
    ...
    @view_config(route_name="c102_002_act", renderer="json")
    def c102_002_act(self):
        req      = self.request
        params   = req.params
        url_dict = req.matchdict
...
#Jurnal Umum           
class C102003View(TuBaseViews):
    def __init__(self, context, request):
        TuBaseViews.__init__(self, context, request)
        self.session['mod'] = 'C102003'

    @view_config(route_name="c102_003", renderer="../../templates/apbd/akskpd/C102003.pt")
    def c102_003(self):
        params = self.request.params
        if self.logged:
            return dict(datas=self.datas)
    ...
    @view_config(route_name="c102_003_frm", renderer="../../templates/apbd/akskpd/C102003_frm.pt")
    def c102_003_frm(self):
        req      = self.request
        params   = req.params
        url_dict = req.matchdict
        self.datas['id'] = 'id' in url_dict and int(url_dict['id']) or 0
    ...
    @view_config(route_name="c102_003_act", renderer="json")
    def c102_003_act(self):
        req      = self.request
        params   = req.params
        url_dict = req.matchdict
...
#Jurnal Koreksi           
class C102004View(TuBaseViews):
    def __init__(self, context, request):
        TuBaseViews.__init__(self, context, request)
        self.session['mod'] = 'C102004'

    @view_config(route_name="c102_004", renderer="../../templates/apbd/akskpd/C102004.pt")
    def c102_004(self):
        params = self.request.params
        if self.logged:
            return dict(datas=self.datas)
    ...
    @view_config(route_name="c102_004_frm", renderer="../../templates/apbd/akskpd/C102004_frm.pt")
    def c102_004_frm(self):
        req      = self.request
        params   = req.params
        url_dict = req.matchdict
        self.datas['id'] = 'id' in url_dict and int(url_dict['id']) or 0
    ...
    @view_config(route_name="c102_004_act", renderer="json")
    def c102_004_act(self):
        req      = self.request
        params   = req.params
        url_dict = req.matchdict
...
#Jurnal Penutup           
class C102005View(TuBaseViews):
    def __init__(self, context, request):
        TuBaseViews.__init__(self, context, request)
        self.session['mod'] = 'C102005'

    @view_config(route_name="c102_005", renderer="../../templates/apbd/akskpd/C102005.pt")
    def c102_005(self):
        params = self.request.params
        if self.logged:
            return dict(datas=self.datas)
    ...
    @view_config(route_name="c102_005_frm", renderer="../../templates/apbd/akskpd/C102005_frm.pt")
    def c102_005_frm(self):
        req      = self.request
        params   = req.params
        url_dict = req.matchdict
        self.datas['id'] = 'id' in url_dict and int(url_dict['id']) or 0
    ...
    @view_config(route_name="c102_005_act", renderer="json")
    def c102_005_act(self):
        req      = self.request
        params   = req.params
        url_dict = req.matchdict
...
#Jurnal Item
class C102997View(TuBaseViews):
    @view_config(route_name="c102_997", renderer="../../templates/apbd/akskpd/C102997.pt")
    def c102_997(self):
        params = self.request.params
        ...
        if self.logged and self.is_akses_mod('read'):
            ...
            if not self.logged:
                ...
                headers=forget(self.request)
                return HTTPFound(location='/c100', headers=headers)

    @view_config(route_name="c102_997_frm", renderer="../../templates/apbd/akskpd/C102997_frm.pt")
    def c102_997_frm(self):
        req = self.request
        params = req.params
        url_dict = req.matchdict
        self.datas['id'] = 'id' in url_dict and int(url_dict['id']) or 0
    ...
    @view_config(route_name="c102_997_act", renderer="json")
    def c102_997_act(self):
        req    = self.request
        params = req.params
        url_dict = req.matchdict
...
#Posting STS           
class C101001View(TuBaseViews):
    def __init__(self, context, request):
        TuBaseViews.__init__(self, context, request)
        self.session['mod'] = 'C101001'

    @view_config(route_name="c101_001", renderer="../../templates/apbd/akskpd/C101001.pt")
    def c101_001(self):
        params = self.request.params
        if self.logged:
            return dict(datas=self.datas)
    ...
    @view_config(route_name="c101_001_act", renderer="json")
    def c101_001_act(self):
        req      = self.request
        params   = req.params
        url_dict = req.matchdict
        ...
        if self.logged :
            ...
            if url_dict['act']=='grid' and self.is_akses_mod('read'):
            ...
            elif url_dict['act']=='postingall' and self.is_akses_mod('postingall'):
                d={}
                d['id'] = pk_id
                d['update_uid'] = self.session['user_id']
                #rows = StsModel.get_posting_all(d) 
                d['posted'] = 1
                rows = StsModel.update(d)
                if rows:
                    self.d['msg']='Sukses Posting Semua Data'
                    self.d['success']=True
                return self.d
                ...
                rows = StsModel.update(d)
                if rows:
                    self.d['msg']='Sukses Un-Posting Data'
                    self.d['success']=True
                return self.d    
...
#Posting SP2D           
class C101002View(TuBaseViews):
    def __init__(self, context, request):
        TuBaseViews.__init__(self, context, request)
        self.session['mod'] = 'C101002'

    @view_config(route_name="c101_002", renderer="../../templates/apbd/akskpd/C101002.pt")
    def c101_002(self):
        params = self.request.params
        if self.logged:
            return dict(datas=self.datas)
    ...
    @view_config(route_name="c101_002_act", renderer="json")
    def c101_002_act(self):
        req      = self.request
        params   = req.params
        url_dict = req.matchdict
...
#Posting Apinvoice           
class C101003View(TuBaseViews):
    def __init__(self, context, request):
        TuBaseViews.__init__(self, context, request)
        self.session['mod'] = 'C101003'

    @view_config(route_name="c101_003", renderer="../../templates/apbd/akskpd/C101003.pt")
    def c101_003(self):
        params = self.request.params
        if self.logged:
            return dict(datas=self.datas)
    ...
    @view_config(route_name="c101_003_act", renderer="json")
    def c101_003_act(self):
        req      = self.request
        params   = req.params
        url_dict = req.matchdict
...
#Posting Arinvoice           
class C101004View(TuBaseViews):
    def __init__(self, context, request):
        TuBaseViews.__init__(self, context, request)
        self.session['mod'] = 'C101004'

    @view_config(route_name="c101_004", renderer="../../templates/apbd/akskpd/C101004.pt")
    def c101_004(self):
        params = self.request.params
        if self.logged:
            return dict(datas=self.datas)
    ...
    @view_config(route_name="c101_004_act", renderer="json")
    def c101_004_act(self):
        req      = self.request
        params   = req.params
        url_dict = req.matchdict
