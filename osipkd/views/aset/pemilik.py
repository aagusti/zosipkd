    ...
    def view_pemilik_add(self):
        ...
        if req.POST:
            if 'simpan' in req.POST:
                ...
                b = a['kode']
                c = "%s" % b
                cek  = DBSession.query(AsetPemilik).filter(AsetPemilik.kode==c).first()
                if cek :
                    self.request.session.flash('Kode sudah ada.', 'error')
                    return HTTPFound(location=self.request.route_url('aset-pemilik-add'))

                try:
                    c = form.validate(controls)
                except ValidationFailure, e:
                    return dict(form=form)              
                    return HTTPFound(location=req.route_url('aset-pemilik-add'))
    ...
    def view_pemilik_edit(self):
        ...
        if request.POST:
            if 'simpan' in request.POST:
                ...
                cek = DBSession.query(AsetPemilik).filter(AsetPemilik.kode==c).first()
                if cek:
                    kode1 = DBSession.query(AsetPemilik).filter(AsetPemilik.id==uid).first()
                    d     = kode1.kode
                    if d!=c:
                        self.request.session.flash('Kode sudah ada.', 'error')
                        return HTTPFound(location=request.route_url('aset-pemilik-edit',id=row.id))
                
                try:
                    c = form.validate(controls)
                except ValidationFailure, e:
                    return dict(form=form)               
                    return HTTPFound(location=request.route_url('aset-pemilik-edit',
                                      id=row.id))
