...
class view_aset_kiba(BaseViews):
    ...
    # CSV #
    ##########    
    @view_config(route_name='aset-kiba-csv', renderer='csv',
                 permission='read')
    def export_csv(self):
        request = self.request
        ses     = self.request.session
        ...
        query = DBSession.query(Unit.kode.label('Kode_Unit'), Unit.nama.label('Nama_Unit'),
                ...
                AsetKib.jumlah.label('Jumlah'), AsetKib.kondisi.label('Kondisi'), AsetKib.kib.label('Kib'), 
                AsetKib.a_luas_m2.label('Luas'), AsetKib.a_sertifikat_nomor.label('No_Sertifikat'), AsetKib.a_sertifikat_tanggal.label('Tgl_Sertifikat'),
                AsetKib.a_alamat.label('Alamat'), AsetKib.a_hak_tanah.label('Status_Tanah'), AsetKib.a_penggunaan.label('Penggunaan')
                ).filter(AsetKib.unit_id == Unit.id,
                       AsetKib.pemilik_id == AsetPemilik.id, 
                       AsetKib.kategori_id==AsetKategori.id,
                       AsetKib.kib=='A', 
        ...
        filename = 'KIB_A%s.csv' % datetime.now().strftime('%Y%m%d%H%M%S')
