...
class AsetKategoriModel(UraianModel, Base):
    ...
    children = relationship('AsetKategoriModel',
                        cascade="all",
                        backref=backref("parent", remote_side='AsetKategoriModel.id'),
                        collection_class=attribute_mapped_collection('uraian'),
    #parent = relationship('AsetKategoriModel', 
    #                      remote_side=[id]
                    ) 
                    
    def __repr__(self):
        return "AsetKategoriModel(uraian=%r, id=%r, parent_id=%r)" % (
                    self.uraian,
                    self.id,
                    self.parent_id
                )            

class AsetKebijakanModel(UraianModel, Base):
    __tablename__ = 'kebijakans'
    __table_args__ = {'extend_existing':True,'schema' : 'aset'}
    kategoris = relationship("AsetKategoriModel", backref="kebijakans")
    minimum = Column(BigInteger, nullable=False)
    tahun = Column(Integer, nullable=False)
    kategori_id = Column(Integer, ForeignKey("aset.kategoris.id"), nullable=False)

class AsetPemilikModel(UraianModel, Base):
    __tablename__ = 'pemiliks'
    __table_args__ = {'extend_existing':True,'schema' : 'aset'}
        
class AsetKibModel(DefaultModel, Base):
    __tablename__ = 'kibs'
    __table_args__ = {'extend_existing':True,'schema' : 'aset'}
    ...
    masa_manfaat = Column(SmallInteger, nullable=False)
    nilai_sisa = Column(BigInteger, nullable=False)
...
class AsetDelModel(UraianModel, Base):
...
class AsetDelItemModel(DefaultModel, Base):
    ...
    kibs = relationship("AsetKibModel", backref="delitems")

class AsetRuangModel(UraianModel, Base):
...
class AsetHistKondisiModel(UraianModel, Base):
...
class AsetHistPemilikModel(UraianModel, Base):
...
class AsetHistSusutModel(UraianModel, Base):
